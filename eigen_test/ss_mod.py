...
# Reboiler:
m.rho_B_CO2 = Var(within=NonNegativeReals,initialize=m.rho_B_CO2_nom) # Density in kg/m^3
...
# Cooler:
m.rho_C_CO2 = Var(within=NonNegativeReals,initialize=m.rho_B_CO2_nom)
m.rho_C_H2O = Var(within=NonNegativeReals,initialize=m.rho_C_H2O_nom)
...
def const_S7_rule(m): 
    ...
    return 0 == 1/m.MS*(m.FL_S*(m.xS_CO2_5 - m.xS_CO2[4]) + \
            (m.FV_S + m.qF*m.FF_S)*(m.yS_CO2[3] - m.yS_CO2[4]))
m.const_S7 = Constraint(rule=const_S7_rule)

# tray 3 (feed):
def const_S8_rule(m):
    return 0 == 1/m.MS*(m.FL_S*(m.xS_CO2[4] - m.xS_CO2[m.NTFS.value]) + \
...
# tray 2:
def const_S9_rule(m):
    return 0 == 1/m.MS*((m.FL_S+ (1-m.qF)*m.FF_S)*(m.xS_CO2[3] - m.xS_CO2[2]) + \
            m.FV_S*(m.yS_CO2[1] - m.yS_CO2[2]))
m.const_S9 = Constraint(rule=const_S9_rule)

# tray 1 (reboiler):
def const_S10_rule(m):
    return 0 == 1/m.MBS*((m.FL_S + (1-m.qF)*m.FF_S)*(m.xS_CO2[2]-m.xS_CO2[1]) + \
...
def const_B7_rule(m):
    return 0 == 1/(m.V_B_tb*m.rho_B_CO2*m.CpB_CO2)*\
            (m.FV_S*m.dH_B - m.U_ov_B*m.A_B*(m.TB_tb_ave - m.TB_sh))
...
def const_B13_rule(m):
    return m.rho_B_CO2 == m.rho_B_CO2_nom
m.const_B13 = Constraint(rule=const_B13_rule)
...
def const_C16_rule(m):
    return m.rho_C_CO2 == m.rho_B_CO2
m.const_C16 = Constraint(rule=const_C16_rule)

# assuming heat capacity of stream is that of CO2
def const_C6_rule(m):
    return 0 == 1/(m.rho_C_CO2*m.V_C_tb*m.CpC_CO2)*\
            (m.F_C*m.CpC_CO2*(m.TC_tb_in-m.TC_tb_out) - \
            m.U_ov_C*m.A_C*(m.TC_tb_ave-m.TC_sh_ave))
...
def const_C17_rule(m):
    return m.rho_C_H2O == m.rho_C_H2O_nom
m.const_C17 = Constraint(rule=const_C17_rule)
...
def const_C11_rule(m):
    return 0 == 1/(m.rho_C_H2O*m.V_C_sh*m.CpC_H2O)*\
            (m.F_C_H2O*m.CpC_H2O*(m.TC_sh_out-m.TC_sh_in) - \
            m.U_ov_C*m.A_C*(m.TC_tb_ave-m.TC_sh_ave))
